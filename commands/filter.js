module.exports = {
   ...
   if (!queue || !queue.playing) return message.channel.send(`${message.author}, Il n’y a pas de musique actuellement !. ❌`);
        ...
        if (!args[0]) return message.channel.send(`${message.author}, Veuillez entrer un nom de filtre valide. ❌\n\`bassboost, 8D, nightcore\``);
        ...
        if (!filter) return message.channel.send(`${message.author}, Je n’ai pas trouvé de filtre avec ton nom. ❌\n\`bassboost, 8D, nightcore\``);
